...
public class CaffeineCache {
     ...
     * @since 2.1.5
     */
    public static <K, V> Cache<K, V> create(int expireAfterAccess) {
    ...
    public static <K, V> Cache<K, V> createExpireAfterWrite(int expireAfterWrite) {
        return Caffeine.newBuilder().expireAfterAccess(expireAfterWrite, TimeUnit.SECONDS).build();
    }
     ...
     * @since 2.1.5
     */
    public static <K, V> Cache<K, V> createExpireAfterWrite(int expireAfterWrite, CacheLoader<K, V> cacheLoader) {
        return Caffeine.newBuilder().expireAfterAccess(expireAfterWrite, TimeUnit.SECONDS).build(cacheLoader);
    }
}
