...
public class UrlUtils {
     ...
     */
    public static URLConnection openConnectionWithTimeout(String urlString) throws IOException, IllegalArgumentException {
        if (urlString == null || urlString.trim().isEmpty()) {
            throw new IllegalArgumentException("URL string cannot be null or empty");
        }
        // 创建URL对象。URL构造函数如果格式错误会抛出 MalformedURLException (是 IOException 的子类)。
        URL url = new URL(urlString);
        // 打开连接。openConnection() 可能会抛出 IOException。
        URLConnection connection = url.openConnection();
        // 设置连接超时和读取超时。这些方法可能会抛出 IllegalArgumentException。
        connection.setConnectTimeout(5000); // 连接超时 5 秒 (等待建立连接的时间)
        connection.setReadTimeout(10000);   // 读取超时 10 秒 (等待读取数据的时间)
        return connection;
    }
     ...
     *
     * <p>此方法内部调用 {@link #openConnectionWithTimeout(String)} 建立连接，
    ...
    public static InputStream getUrlInputStream(String urlString) throws IOException, IllegalArgumentException {
        // 调用公共方法建立连接
        URLConnection connection = openConnectionWithTimeout(urlString);
     ...
     *
     * <p>此方法内部调用 {@link #openConnectionWithTimeout(String)} 建立连接。</p>
