...
public class HsInputStream extends FilterInputStream {
    ...
    private final HsProcessState rr = new HsProcessState();
    ...
    private State readBackref(HsProcessState rr) {
    ...
    private State readLiteral(HsProcessState rr) throws IOException {
        if (rr.off < rr.end) {
            int bits = getBits(8);
            if (bits == -1) return State.BUFFER_EMPTY;
            int mask = (1 << windowSize)  - 1;
